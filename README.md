...
1. **Per-Player Chat Recipient Filtering (ChatControl Integration)**
   ...
   - **Fix:** `OnAsyncPlayerChat`, `PlayableSound`, and `PlayableRichSound` now accept and filter by `event.getRecipients()`. Players who turned off their chat will **NEVER** hear chat notification sounds from other players.

2. **Strict Event Cancellation (Server Mute Fix)**
...
3. **Thread-Safe & Exception-Safe Listener Iteration**
...
4. **Updated PaperMC Repositories & Compatibility**
